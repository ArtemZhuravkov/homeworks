// ЗАПРОСИТЬ ВОЗРАСТ У ПОЛЬЗОВАТЕЛЯ
...
var result = (birth <= 2000) ? alert('Welcome') :
    ...
    (birth != Number) ? alert('Enter a number!') : alert('hello');
...
var currency = prompt('Выберите валюту в которую хотите конвертировать(EUR, USD, GBP)');
var amount = prompt('введите сумму в "UAH"');
var eur = Math.round(amount / 26),
    usd = Math.round(amount / 24),
    gbp = Math.round(amount / 28.30);
